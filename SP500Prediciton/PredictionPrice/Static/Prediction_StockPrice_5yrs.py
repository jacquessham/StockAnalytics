...
for curr_ticker in sp500_companies:
	...
	query = """select tradedate, closeprice from stock.stockprice
	           ...
	           between 2013 and 2018""".format(curr_ticker)
	...
	for index, row in pred.iterrows():
		try:
			insert_pred(conn, 'pred_price_sp500_5yr', curr_ticker, row)
		except:
			print('####################')
...
error_log.to_csv('ETLPipelines/Logs/ErrorLog_SP500Pred5yr.csv', index=False)

# Save timer log
time_log = pd.DataFrame(time_log)
time_log.to_csv('ETLPipelines/Logs/TimeLog_SP500Pred5yr.csv', index=False)
